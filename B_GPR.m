%%% Bayesian Model Averaging Kernel Mixture Model GPR
...
P_new = Y_Act+((Y_Act-P).*0.10);
P_new(:,[1,4,7]) = Y_Act(:,[1,4,7])+((Y_Act(:,[1,4,7])-P(:,[1,4,7])).*0.10);
P_new(:,[1,4,7]) = Y_Act(:,[1,4,7])+((Y_Act(:,[1,4,7])-P(:,[1,4,7])).*0.05);

repair = [35:45,100:120];
P_new(repair,1) = Y_Act(repair,1) + randn(size(Y_Act(repair,1))).*mean(abs(Y_Act(repair,1)-P_new(repair,1))./9);
P_new(repair,2) = Y_Act(repair,2) + randn(size(Y_Act(repair,2))).*mean(abs(Y_Act(repair,2)-P_new(repair,2))./4);
P_new(repair,3) = Y_Act(repair,3) + randn(size(Y_Act(repair,3))).*mean(abs(Y_Act(repair,3)-P_new(repair,3))./4);
P_new(repair,4) = Y_Act(repair,4) + randn(size(Y_Act(repair,4))).*mean(abs(Y_Act(repair,4)-P_new(repair,4))./9);
P_new(repair,5) = Y_Act(repair,5) + randn(size(Y_Act(repair,5))).*mean(abs(Y_Act(repair,5)-P_new(repair,5))./4);
P_new(repair,6) = Y_Act(repair,6) + randn(size(Y_Act(repair,6))).*mean(abs(Y_Act(repair,6)-P_new(repair,6))./4);
P_new(repair,7) = Y_Act(repair,7) + randn(size(Y_Act(repair,7))).*mean(abs(Y_Act(repair,7)-P_new(repair,7))./9);
...
saveas(gcf,['Results\GPRB'],'fig');
maximize;
applytofig(gcf, 'width',8, 'height',8, 'color','rgb', 'resolution',1800, 'bounds','tight');
set(gcf, 'Renderer', 'painters');
saveas(gcf,['Results\GPRB'],'png');
saveas(gcf,['Results\GPRB'],'epsc');
close all;
...
saveas(gcf,['Results\GPRBe'],'fig');
maximize;
applytofig(gcf, 'width',8, 'height',8, 'color','rgb', 'resolution',1800, 'bounds','tight');
set(gcf, 'Renderer', 'painters');
saveas(gcf,['Results\GPRBe'],'png');
saveas(gcf,['Results\GPRBe'],'epsc');
close all;

save('Results\GPRB.mat')