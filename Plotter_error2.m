...
h(1) = plot(timings,P(:,4), Styles{4},'Color',rgb(Colors(4)),'LineWidth',0.5);
h(2) = plot(timings,P(:,5), Styles{5},'Color',rgb(Colors(5)),'LineWidth',0.5);
hl = legend('BMA-MKSVR','BMA-MKGPR',4);
set(hl,'Color','w');
axis('off');
set(h,'Visible','Off');

dir = strcat(cd,'\Figs\');
saveas(gcf,[dir,'Error2'],'fig');
maximize;
applytofig(gcf, 'width',8, 'height',8, 'color','rgb', 'resolution',1800, 'bounds','tight');
set(gcf, 'Renderer', 'painters');
saveas(gcf,[dir,'Error2'],'png');
saveas(gcf,[dir,'Error2'],'epsc');
close all;
