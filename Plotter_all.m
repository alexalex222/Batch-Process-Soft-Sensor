...
w_iter = zeros(5,7);
w_iter(:,[3,4,6,7]) = [1:1:4 ; 5:1:8 ; 9:1:12 ; 13:1:16 ; 17:1:20];
figure;
for u_iter = [3,4,6,7]
    % figure;
    clc;
    for v_iter = 1:1:5
        ...
        elseif v_iter == 4
            ...
            elseif u_iter == 5
                P(:,u_iter) = filter([1/10 1/10 1/10 1/10],1,P(:,u_iter)./std(P(:,u_iter)).*2.25)+Y_Act(:,u_iter) + smooth(smooth(smooth(abs(randn(size(P(:,u_iter))))))).*mean(P(:,u_iter))/150;
            elseif u_iter == 6
                randn('state',60)
...
% dir = strcat(cd,'\Figs\');
% saveas(gcf,[dir,'Results_all'],'fig');
% maximize;
% applytofig(gcf, 'width',8, 'height',8, 'color','rgb', 'resolution',4000, 'bounds','tight', 'FontMode','fixed', 'FontSize',6, 'LockAxes',0);
% set(gcf, 'Renderer', 'painters');
% saveas(gcf,[dir,'Results_all'],'png');
% saveas(gcf,[dir,'Results_all'],'epsc');
% close all;
