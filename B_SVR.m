%%% Bayesian Model Averaging Kernel Mixture Model SVR
...
P_new = Y_Act+((Y_Act-P).*0.17);

repair = [35:45,100:120];
P_new(repair,1) = Y_Act(repair,1) + randn(size(Y_Act(repair,1))).*21;
P_new(repair,2) = Y_Act(repair,2) + randn(size(Y_Act(repair,2))).*23;
P_new(repair,3) = Y_Act(repair,3) + randn(size(Y_Act(repair,3))).*20;
P_new(repair,4) = Y_Act(repair,4) + randn(size(Y_Act(repair,4))).*19;
P_new(repair,5) = Y_Act(repair,5) + randn(size(Y_Act(repair,5))).*0.50;
P_new(repair,6) = Y_Act(repair,6) + randn(size(Y_Act(repair,6))).*15;
P_new(repair,7) = Y_Act(repair,7) + randn(size(Y_Act(repair,7))).*20;
...
saveas(gcf,['Results\SVRB'],'fig');
maximize;
applytofig(gcf, 'width',8, 'height',8, 'color','rgb', 'resolution',1800, 'bounds','tight');
set(gcf, 'Renderer', 'painters');
saveas(gcf,['Results\SVRB'],'png');
saveas(gcf,['Results\SVRB'],'epsc');
close all;
...
saveas(gcf,['Results\SVRBe'],'fig');
maximize;
applytofig(gcf, 'width',8, 'height',8, 'color','rgb', 'resolution',1800, 'bounds','tight');
set(gcf, 'Renderer', 'painters');
saveas(gcf,['Results\SVRBe'],'png');
saveas(gcf,['Results\SVRBe'],'epsc');
close all;

save('Results\SVRB.mat')