...
for iii_iter = 1:1:length(Y_Vars)
    ...
    for jjj_iter = 5:-1:1
        ...
        if jjj_iter == 5
            ...
            plot([min(Y_plot).*0.80,max(Y_plot)*1.20],[min(Y_plot),max(Y_plot)], 'Color',rgb('Black'),'LineWidth',1.5);
        end
        P_plot = (P(:,iii_iter)-repmat(Mu,length(P(:,iii_iter)),1))./repmat(Sigma,length(P(:,iii_iter)),1);
        plot(Y_plot,P_plot, Styles{jjj_iter},'Color',rgb(Colors(jjj_iter)),'LineWidth',0.5);
        ...
        if iii_iter ~= 5
        y_lim = get(gca,'ylim');
        ylim([y_lim(1)-0.5,y_lim(end)+0.5]);
        x_lim = get(gca,'xlim');
        xlim([x_lim(1)-0.5,x_lim(end)+0.5]);
        else
        y_lim = get(gca,'ylim');
        ylim([y_lim(1)-0.5,y_lim(end)+0.5]);
        x_lim = get(gca,'xlim');
...
hl = legend('NN','SVR','GPR','B-SVR','B-GPR',4);
set(hl,'Color','w');
axis('off');
set(h,'Visible','Off');

dir = strcat(cd,'\Figs\');
saveas(gcf,[dir,'Correlation'],'fig');
maximize;
applytofig(gcf, 'width',8, 'height',8, 'color','rgb', 'resolution',1800, 'bounds','tight');
set(gcf, 'Renderer', 'painters');
saveas(gcf,[dir,'Correlation'],'png');
saveas(gcf,[dir,'Correlation'],'epsc');
close all;
